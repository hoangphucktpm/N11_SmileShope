	...
	public void themLoaiSP() {
		...
		if (moi != null && !moi.equalsIgnoreCase("")) {
			...
			if (check == true) {
				...
				JOptionPane.showMessageDialog(this, "Thêm loại sản phẩm không thành công");
		} else
			JOptionPane.showMessageDialog(this, "Thêm loại sản phẩm không thành công");
...
//		String hinhAnh = (String) cbohinhanh.getSelectedItem();
	...
	public sanPham valiData() {
		...
		if (txtTenSP.getText().isEmpty() || txtTenSP.getText().trim() == "") {
			ShowErrorField("Tên sản phẩm không được rỗng", txtTenSP);
			return null;
		}
		String tenSP = txtTenSP.getText().trim();
		if (txtGiaNhap.getText().isEmpty() || txtGiaNhap.getText().trim() == "") {
			ShowErrorField("Giá nhập không được rỗng", txtGiaNhap);
			return null;
		} else if (txtGiaNhap.getText().trim().matches("([1-9][0-9]*)") == false) {
			ShowErrorField("Vui lòng nhập giá nhập lớn hơn 0 !", txtGiaNhap);
			return null;
		}
		String giaNhap = txtGiaNhap.getText().trim();
		if (txtSoLuong.getText().isEmpty() || txtSoLuong.getText().trim() == "") {
			ShowErrorField("Số lượng không được rỗng", txtSoLuong);
			return null;
		} else if (txtSoLuong.getText().trim().matches("([1-9][0-9]*)") == false) {
			ShowErrorField("Vui lòng nhập Số lượng lớn hơn 0 !", txtSoLuong);
			return null;
		}
		String soLuong = txtSoLuong.getText().trim();
		java.util.Date ngayNhap = txtChonNgaynhap.getDate();
		if (ngayNhap == null) {
			JOptionPane.showMessageDialog(this, "Ngày nhập không được rỗng");
			return null;
		}
		try {
			sp = new sanPham(maSP, tenSP, Double.parseDouble(giaNhap), Integer.parseInt(soLuong), ngayNhap);
			return sp;
