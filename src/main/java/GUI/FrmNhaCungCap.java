...
public class FrmNhaCungCap extends JFrame implements ActionListener, MouseListener{
	...
	public FrmNhaCungCap() {
		...
		pnlThongTinNCC.setBounds(20, 48, 1286, 208);
		...
		pnTable.setBounds(20, 299, 1286, 485);
		...
		scrDSHD.setBounds(10, 108, 1266, 367);
		pnTable.add(scrDSHD);
		...
		rdMNCC.addActionListener(new ActionListener() {
		...
		rdTenNCC.addActionListener(new ActionListener() {
		...
		rdE.addActionListener(new ActionListener() {
		...
		rdStatus.addActionListener(new ActionListener() {
...
//									n++,x.getMaNhaCungCap(),x.getTenNhaCungCap(),x.getSdt(),x.getEmail(),x.getDiaChi()
//							});
//							}
//						}
						updateCBBox();
	...
	public void luuThayDoi ()
	{
		...
		else
		{
			try {
				...
				{
					boolean newL = dao.them(tenNCCString, Sdt, email, diaChi, tinhTrang);
					 if(newL == true)
					 {
						 btnThem.setText("Thêm");
						 xoaTrang();
...
//Tìm thông tin nhà cung cấp	
	public void tim()
	{
		...
		try {
			tim = comboBox.getSelectedItem().toString();
		...
		xoaAllDataTable();
			...
			else if (rdStatus.isSelected())
			{
				...
				for (NhaCungCap x: list)
				{
				if(tim.equalsIgnoreCase("Đang hợp tác"))
				{
					if(x.getTinhTrang() == 1)
					{
						tablemodel.addRow(new Object[] {
...
//	Kiểm tra thông tin nhà cung cấp trước khi thêm
	public NhaCungCap valiData() {
		...
		else if (txtE.getText().trim().matches("^\\b[\\w.%+-]+@[\\w.-]+\\.com\\b$") == false)
		{
